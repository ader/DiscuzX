...
/**********************    密码配置区 开始*******************************/
$tool_password = 'FT^GY&HU*'; // ☆★☆★☆★ 请您设置一个工具包的高强度密码，不能为空！☆★☆★☆★
/**********************    密码配置区 结束*******************************/
error_reporting(E_ALL); //E_ERROR | E_WARNING | E_PARSE | E_ALL
@set_time_limit(0);
define('TOOLS_ROOT', dirname(__FILE__) . "/");
define('VERSION', '2009');
define('Release', '100120');
$functionall = array(array('all', 'all_repair', '检查或修复数据库', '对所有数据表进行检查修复工作。'), array('all', 'all_runquery', '快速设置(SQL)', '可以运行任意SQL语句，请慎用。'), array('all', 'all_checkcharset', '字段编码检测', '对所有数据表进行编码检查和修复。'), array('all', 'all_config', '修改配置文件', '配置文件修改助手'), array('all', 'all_restore', '恢复数据库备份', '恢复论坛数据备份。'), array('all', 'all_setadmin', '找回管理员', '将把您指定的会员设置为管理员，也可以重新设置密码。'), array('dz', 'dz_filecheck', '文件校验', '检查论坛程序目录下的非Discuz!官方文件。'), array('dz', 'dz_rplastpost', '修复最后回复', '修复版块最后回复。'), array('dz', 'dz_rpthreads', '批量修复主题', '某些帖子页面会出现未定义操作，可以用批量修复主题的功能修复下。'), array('dz', 'dz_mysqlclear', '数据库冗余清理', '对您的数据进行有效性检查，删除冗余数据信息。'), array('dz', 'dz_moveattach', '附件保存方式', '将您现在的附件存储方式按照指定方式进行目录结构调整并重新存储。'), array('dz_uch', 'uch_dz_replace', '应用过滤规则', '按照设置的词语过滤列表，可选择性的对所有内容进行处理,内容将按照过滤规则进行处理。'), array('all', 'all_updatecache', '<font color=red>更新缓存</font>', '清除缓存。'),);
$toolbar = array(array('phpinfo', 'INFO'), array('datago', '转码'), array('all_logout', '退出'),);
//初始化
$plustitle = '';
$lockfile = '';
...
if (($whereis == 'is_dz' || $whereis == 'is_dzx') && !defined('DISCUZ_ROOT')) {
	define('DISCUZ_ROOT', TOOLS_ROOT);
}
if (!$whereis && !in_array($whereis, array('is_dzx', 'is_dz', 'is_uc', 'is_uch', 'is_ss'))) {
	$alertmsg = '';
...
if ($action == 'all_repair') {
	...
	$backdirarray = array( //不同的程序存放备份文件的目录是不同的
		'is_dz' => 'forumdata', 'is_uc' => 'data/backup', 'is_uch' => 'data', 'is_ss' => 'data');
			...
			while ($entry = $dir->read()) {
				...
				if (is_file($entry) && preg_match("/\.sql/i", $entry)) {
					...
					if (preg_match("/\-1.sql/i", $entry) || $identify[3] == 'shell') {
						$exportlog[$identify[0]] = array('version' => $identify[1], 'type' => $identify[2], 'method' => $identify[3], 'volume' => $identify[4], 'filename' => $entry, 'size' => $filesize);
					}
					...
					if (preg_match("/backup\_/i", $entry)) {
						$exportlog[filemtime($entry)] = array('version' => $bakidentify[1], 'type' => $bakidentify[2], 'method' => $bakidentify[3], 'volume' => $bakidentify[4], 'bakentry' => $identify['bakentry'], 'filename' => $entry);
					}
				}
			}
			$dir->close();
	...
	$action        = array('1' => '冗余回复数据清理', '2' => '冗余附件数据清理', '3' => '冗余会员数据清理', '4' => '冗余板块数据清理', '5' => '主题信息清理', '6' => '完成数据冗余清理');
	$steps         = count($action);
	...
	if ($step == 0) {
		...
		<table>
			<tr>
				<th width="30%">Posts表的清理</th>
				<td>[<a href="?action=dz_mysqlclear&step=1&stay=1">单步清理</a>]</td>
			</tr>
			<tr>
				<th width="30%">Attachments表的清理</th>
				<td>[<a href="?action=dz_mysqlclear&step=2&stay=1">单步清理</a>]</td>
			</tr>
			<tr>
				<th width="30%">Members表的清理</th>
				<td>[<a href="?action=dz_mysqlclear&step=3&stay=1">单步清理</a>]</td>
			</tr>
			<tr>
				<th width="30%">Forums表的清理</th>
				<td>[<a href="?action=dz_mysqlclear&step=4&stay=1">单步清理</a>]</td>
			</tr>
			<tr>
				<th width="30%">Threads表的清理</th>
				<td>[<a href="?action=dz_mysqlclear&step=5&stay=1">单步清理</a>]</td>
			</tr>
			<tr>
				<th width="30%">所有表的清理</th>
				<td>[<a href="?action=dz_mysqlclear&step=1&stay=0">全部清理</a>]</td>
			</tr>
		</table>
		<?php
		specialdiv();
	...
	if (isset($replacesubmit) || $start > 0) {
		...
		if ($converted || $end < $maxid) {
			...
			if ($threads_banned > 0) {
				echo "<tr><td><li>" . $threads_banned . "个主题被放入回收站.</li></td></tr>";
			}
			if ($threads_mod > 0) {
				echo "<tr><td><li>" . $threads_mod . "个主题被放入审核列表.</li></td></tr>";
			}
			if ($posts_mod > 0) {
				echo "<tr><td><li>" . $posts_mod . "个回复被放入审核列表.</li></td></tr>";
			}
			echo "<tr><td><li>替换了" . $convertedrows . "条记录</li></td></tr>";
			echo "</table>";
		...
		<form action="?action=all_setadmin" method="post">
			<table>
				...
				if ($whereis != 'is_uc') {
					...
					<tr>
						<th width="30%"><input class="radio" type="radio" name="loginfield"
											   value="<?php echo $username; ?>" checked>用户名<input class="radio"
																								  type="radio"
																								  name="loginfield"
				...
				if ($secq) {
					...
					<tr>
						...
						<td width="70%"><input class="radio" name="issecques" value="1" checked="checked" type="radio">是&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input
								name="issecques" value="" class="radio" type="radio">否
						</td>
					</tr>
				<?php
				}
	...
	echo "<h4>附件保存方式</h4>";
	$atoption = array('0' => '标准(全部存入同一目录)', '1' => '按论坛存入不同目录', '2' => '按文件类型存入不同目录', '3' => '按月份存入不同目录', '4' => '按天存入不同目录',);
				...
				if ($newattachsave) {
					switch ($newattachsave) {
						...
						case 2:
							...
							break;
						case 3:
							$attach_subdir = 'month_' . gmdate('ym', $dateline);
							break;
						case 4:
							$attach_subdir = 'day_' . gmdate('ymd', $dateline);
							break;
					}
...
function getplace() {
	...
	$whereis = FALSE;
	if (is_writeable('./config/config_global.php') && is_dir('source')) { //判断Discuz!目录
		$whereis  = 'is_dzx';
		$lockfile = './data/tools.lock';
		$cfgfile  = './config/config_global.php';
		$docdir   = './data';
	}
...
function getlistarray($whereis, $type) {
	...
	if ($whereis == 'is_dz' && $dz_version >= '710') {
		if ($type == 'datago') {
			$list = array(array('advertisements', 'parameters', 'advid'), array('request', 'value', 'variable'), array('settings', 'value', 'variable'),);
		}
	} elseif ($whereis == 'is_uch' && $uch_version >= '15') {
		if ($type == 'datago') {
			$list = array(array('ad', 'adcode', 'adid'), array('blogfield', 'tag', 'blogid'), array('blogfield', 'related', 'blogid'), array('feed', 'title_data', 'feedid'), array('feed', 'body_data', 'feedid'), array('share', 'body_data', 'sid'),);
		}
	} elseif ($whereis == 'is_uc') {
		if ($type == 'datago') {
			$list = array(array('feed', 'title_data', 'feedid'), array('feed', 'body_data', 'feedid'), array('settings', 'v', 'k'),);
		}
	} elseif ($whereis == 'is_ss' && $ss_version >= 70) {
		if ($type == 'datago') {
			$list = array(array('ads', 'parameters', 'adid'), array('blocks', 'blocktext', 'blockid'),);
		}
	}
